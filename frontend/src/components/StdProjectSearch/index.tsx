...
export default function ProjectSearch({ initialValue = '', onSubmit, onClose }) {
  ...
  return (
    <div className={'psearch-backdrop'} onClick={onClose}>
      <div
        className={'psearch-panel'}
        role="dialog"
        aria-modal="true"
        aria-label="Search projects"
        onClick={e => e.stopPropagation()}
      >
        <button type="button" className={'psearch-close'} onClick={onClose} aria-label="Close search">✕</button>

        <div className={'psearch-header'}>
          <div className={'psearch-eyebrow'}>// search</div>
          <h3 className={'psearch-title'}>Find a project</h3>
        </div>

        <form onSubmit={handleSubmit} className={'psearch-form'}>
          <input
            ref={inputRef}
            type="text"
            ...
            spellCheck={false}
          />
          <div className={'psearch-actions'}>
            <button type="button" className={'psearch-cancelBtn'} onClick={onClose}>
              ✕ cancel
            </button>
            <button type="submit" className={'psearch-searchBtn'}>
              search
            </button>
          </div>
        </form>

        <p className={'psearch-hint'}>
